...
if __name__ == "__main__":
    ...
    kills = 0
    debug = False
    font_count = pygame.font.SysFont("Papyrus", 150)
    font_norm = pygame.font.SysFont("Papyrus", 40)
    font_end   = pygame.font.SysFont("Papyrus", 100)
    ...
    def hit(dmg, x, y, enemy_i):
        global player_health, player_x, player_y, last_hit, invince, running
    ...
    while running:
        ...
        wave_text = font_norm.render(f"Wave - {wave}", True, (255, 255, 255))
        screen.blit(wave_text, (10, 10))
        kill_text = font_norm.render(f"Kills - {kills}", True, (255, 255, 255))
        screen.blit(kill_text, (10, 80))
        hp_text = font_norm.render(f"Health - {player_health} HP", True, (255, 0, 0))
        screen.blit(hp_text, (WIDTH - 350, 10))

        if now < 1000:
            count_text = font_count.render(f"Three...", True, (255, 255, 255))
            screen.blit(count_text, (200, 350))
        elif now >= 1000 and now < 2000:
            count_text = font_count.render(f"Two...", True, (255, 255, 255))
            screen.blit(count_text, (250, 350))
        elif now >= 2000 and now < 3000:
            count_text = font_count.render(f"One...", True, (255, 255, 255))
            screen.blit(count_text, (250, 350))
        elif now >= 3000 and now < 4000:
            count_text = font_count.render(f"Draw!", True, (255, 255, 255))
            screen.blit(count_text, (250, 350))

        entities = 1 + len(enemies) + len(bullets) + len(enemy_bullets)
        if debug == True:
            debug_text = font_norm.render(f"Entities - {entities}    FPS - {round(clock.get_fps(), 1)}", True, (255, 255, 255))
            screen.blit(debug_text, (10, 920))

        if wave == 31:
            ask_text = font_norm.render(f"End Run - (R)", True, (255, 255, 255))
            screen.blit(ask_text, (250, 350))
            aske_text = font_norm.render(f"Go to Endless - (N)", True, (255, 255, 255))
    ...
    while end_running:
        ...
        wave_text = font_end.render(f"Final Wave - {wave}", True, (255, 255, 255))
        screen.blit(wave_text, (200, 350))

        kill_text = font_end.render(f"Total Kills - {kills}", True, (255, 255, 255))
