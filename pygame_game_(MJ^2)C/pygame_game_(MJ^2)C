...
# Name - 0, HP - 1, Damage - 2, Speed - 3, ATK Speed - 4, Last time hit player - 5, Last shot - 6, Last Summon - 7
enemy_stats = [
    ["Chaser", 40, 5, 2.5, 1, 0, 0, 0],
    ["Shooter", 30, 6, 1, 1, 0, 0, 0],
    ["Chaser", 40, 5, 2.5, 1, 0, 0, 0],
]
...
def new_wave():
    ...
    if wave % 5 == 0 and wave % 10 != 0:
        ...
        enemy_stats.append(["Boss", boss[1][0], boss[1][1], boss[1][2], boss[1][3], 0, 0, 0])
    elif wave > 30:
        wave -= 1
        running = False
    else:
...
while running:
    ...
    for bullet in enemy_bullets[:]:
        ...
        if isinstance(bullet, BossBullet) and bullet_rect.colliderect(player_rect):
            hit(boss[1][1], player_x, player_y, -1)
            enemy_bullets.remove(bullet)
    ...
    for i, enemy in enumerate(enemies):
        ...
        if 3000 - now > 0:
            ...
            if enemy.y + randy <= 50:
                ...
                enemy.y += randy
        if enemy_stats[i][0] == "Boss" and now - enemy_stats[i][6] >= 7000:
        ...
        if enemy_stats[i][0] == "Boss" and now - enemy_stats[i][7] >= 6000:
